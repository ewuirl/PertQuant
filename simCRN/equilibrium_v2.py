...
def AA_interactions(AA_array, F, n, N):
	...
	# Handle case with only 2 strands and 1 interaction
	if len(AA_array) == 1:
		F = F - AA_array[0]

	# Handle case with > 2 strands and > 1 interaction
	elif len(AA_array) > 1:
...
def gen_x_array(*array_args):
	...
	if len(array_args) == 5:
		...
		flat_AA_array = flatten_AA_array(AA_array)
		flat_BB_array = flatten_AA_array(BB_array)
	...
	if len(array_args) == 9:
		...
		flat_AA_array = flatten_AA_array(AA_array)
		flat_BB_array = flatten_AA_array(BB_array)
		flat_CC_array = flatten_AA_array(CC_array)
...
def unpack_x_array(x_array, *args):
	...
	if len(args) == 5:
		...
		AA_shape = np.shape(KAA_array)
		BB_shape = np.shape(KBB_array)

		# Make arrays to store in-set interactions in
		AA_array = np.empty(AA_shape,dtype='object')
		BB_array = np.empty(BB_shape,dtype='object')
	...
	if len(args) == 9:
		...
		AA_shape = np.shape(KAA_array)
		BB_shape = np.shape(KBB_array)
		CC_shape = np.shape(KCC_array)

		# Make arrays to store in-set interactions in
		AA_array = np.empty(AA_shape,dtype='object')
		BB_array = np.empty(BB_shape,dtype='object')
		CC_array = np.empty(CC_shape,dtype='object')
