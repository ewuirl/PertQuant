...
def test_sequence(sequence, test_name, tol, answer):
    ...
    error_list = []
    barcode_ID, is_error = analyze_sequence(sequence, '@seqid', count_array, error_list, tol=tol)
    passed = 1
...
def test_sequence_print(sequence, test_name, tol, answer):
    ...
    if barcode_ID == true_ID:
        ...
        print('True counts: {}'.format(true_ID))
        print('Calc counts: {}'.format(barcode_ID))
    return passed
...
unID_endseq = testseq('unidentified end subsequence', \
    ...
    + 'whatisthis', np.array([1,0]), 1, 1, 0)

short = testseq('short', 'whatisthis', np.zeros(2), 2, 1, -2)

unidentified_seq = testseq('unidentified sequence', \
    'whatisthiswhatisthiswhatisthiswhatisthiswhatisthi', np.array([0,0]), 2, 1, -2)
...
error_seq_list = [mixed_bar, mixed_barc, no_bar, unID_subseq, unID_endseq, short, \
unidentified_seq] 
