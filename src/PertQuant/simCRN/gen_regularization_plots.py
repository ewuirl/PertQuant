...
def gen_regularization_plots_main():
    ...
    parser.add_argument('case_name', type=str, help='The name of the data set, \
        eg from case_name_data.txt')
    ...
    # Read in data
    settings_dict = read_detailed_eq_data_file(f'{case_name}_data.txt')
    N = settings_dict['N']
    M = settings_dict['M']
    L = settings_dict['L']
    ...
    save_file = f'{N}-{M}-{L}_{case}_raw-data'
    raw_data_plot = plot_raw_data(settings_dict, f'D=B=T={N} {case_title} Raw Data', 0.9, save_file=save_file, \
                                  save=save_plot)
    if show_plot:
        plt.show(block=True)
    ...
    dataset_csv = pd.read_csv(f'{N}-{M}-{L}_{case}_dataset_partitions.csv', index_col=0)
    data_dict = get_partitioned_data(A_out_array, Ci_array, dataset_csv)

    # Get regression results
    results_df = pd.read_csv(f'{N}-{M}-{L}_{case}_results_summary_{model_type}.csv', index_col=0)
    dataset_size_list = results_df['dataset_size'].to_numpy()

    # Get trained models
    model_list = []
    for i, dataset_size in enumerate(dataset_size_list):
        model_file_name = f'{folder}/{N}-{M}-{L}_{case}_{model_type}_{dataset_size}.pkl'
    ...
    for i, dataset_size in enumerate(dataset_size_list):
        ...
        train_save = f'{folder}/{N}-{M}-{L}_{case}_train_true_vs_pred_best_{model_type}_{dataset_size}'
        ...
        test_save = f'{folder}/{N}-{M}-{L}_{case}_test_true_vs_pred_best_{model_type}_{dataset_size}'
        ...
        train_save = f'{folder}/{N}-{M}-{L}_{case}_train_residuals_best_{model_type}_{dataset_size}'
        ...
        test_save = f'{folder}/{N}-{M}-{L}_{case}_test_residuals_best_{model_type}_{dataset_size}'
    ...
    r2_save_file=f'{N}-{M}-{L}_{case}_{model_type}_summary_plot_all_r2'
    plot_metric_summary(results_df, case_title, model_type, 'r2', r2_save_file, save=save_plot)

    # Plot the overall MAE results
    MAE_save_file=f'{N}-{M}-{L}_{case}_{model_type}_summary_plot_all_mae'
